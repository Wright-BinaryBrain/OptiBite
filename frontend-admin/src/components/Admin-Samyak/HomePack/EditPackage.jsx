...
function EditPackage(props) {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/api/v1/getPackage/${props.editId}`, {
        withCredentials: true,
      })
      .then((res) => {
        setPackageName(res.data.data.packageName);

        const ids = res.data.data.packageProductId;
        const prod = data.filter((prev) => ids.includes(prev._id));
        setAddedProd(prod);
      })
      .catch((err) => console.log(err));
  }, [props.editId]);
  ...
  function addProduct(id) {
    ...
    if (newProduct) {
      setAddedProd((prevData) => [...prevData, newProduct]);
    }
    setSearch("");
  ...
  function PostPackage(productId) {
    ...
    axios
      .patch(
        `http://localhost:4000/api/v1/updatePackage/${props.editId}`,
        postData,
        {
          headers: {
                  ...
                  {addedProd.map((prod) => {
                    return (
                      <tr>
                        <td>
                          <img
                            src={`http://localhost:4000/uploads/${prod.image[0]}`}
                            height={"50px"}
                            width={"50px"}
                          />
                        </td>
