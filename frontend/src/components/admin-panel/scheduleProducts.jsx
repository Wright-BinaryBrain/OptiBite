...
const ScheduleProducts = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get(
        "http://127.0.0.1:4000/api/v1/getAllScheduledOrders",
        {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      await axios.patch(
        `http://127.0.0.1:4000/api/v1/updateScheduledOrder/${orderId}`,
  ...
  const deleteOrder = async (orderId) => {
    try {
      await axios.delete(
        `http://127.0.0.1:4000/api/v1/deleteScheduledOrder/${orderId}`,
        {
          withCredentials: true,
        }
      );
