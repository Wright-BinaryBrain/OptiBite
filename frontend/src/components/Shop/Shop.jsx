...
function Shop(props) {
    ...
    // useEffect(()=>{
    //     axios.get("https://backend.sabjiland.com/api/v1/getProducts", {params: {
    ...
    useEffect(() => {
        axios.get("https://backend.sabjiland.com/api/v1/getHighestAndLowestRate")
        .then((res)=>(setInitialPrice([Number(res.data.data.lowest),Number(res.data.data.highest)])))
    ...
    function comboHandler(event, filterParam) {
        if (filterParam[0] === "priceChange") {
            if (event.type === "mousemove" || event.type === "mousedown"  || event.type === "touchstart" || event.type === "touchmove") {
        ...
        else if (filterParam[0] === "productFamilyId" || filterParam[0] === "productTypeId"){
            if (event.target.checked) {
                selectedFilters.current.push(filterParam);
                for (let i = 0; i < selectedFilters.current.length; i++) {
                    if(filterParam[0] === "productTypeId") {
                        if(document.querySelector('#family-checkbox-'+String(filterParam[2])).checked === false){
                            document.querySelector('#family-checkbox-'+String(filterParam[2])).click();
                        }
                    }
                }
                ...
                for (let i = 0; i < selectedFilters.current.length; i++) {
                    if (filterParam[0] === selectedFilters.current[i][0] && filterParam[1] === selectedFilters.current[i][1]){
        ...
        if (filterParam[0] === "clickPageNum"){
            ...
            .then((res)=>(setProductList(res.data.data), setProductCount(res.data.count), res.data.count !== 0 ? document.getElementById("pg-num1").click(): null, console.log(res.data)))
            .catch((err)=>console.log(err))
        }
    ...
    useEffect(()=>{
        ...
        if (props.searchItem !== "") {
            ...
            .then((res)=>(setProductList(res.data.data), setProductCount(res.data.count), res.data.count !== 0 ? document.getElementById("pg-num1").click(): null))
            .catch((err)=>console.log(err))
        }
        else {
            axios.get("https://backend.sabjiland.com/api/v1/getProducts", {params: {
    ...
    <div className="shop-container">
        <div className="filter-section">
            ...
            <ShopFilter filterChange={comboHandler} initialPrice={initialPrice} priceRangeValue={priceRangeValue} priceChange={comboHandler} vegNonVeg={veg_nonVeg.current[0]}/>
        </div>
        <div className="shop-section">
            <ShopSort productsPerPage={comboHandler} sortProducts={comboHandler} filterPopup={handleFilterPopup}/>
            ...
            <div className="page-numbers-container">
                <BsArrowLeft style={currentPage !== 1 ? {fontSize: "26px", cursor: "pointer"} : {fontSize: "26px", opacity: "0", pointerEvents: "none"}} onClick={prevPageHandler}/>
                <div className="page-numbers">
                    {
                        pageNumbers.map((itemValue, itemIndex, array) => {
                            if (itemValue -2 === 1 && currentPage === 1) {
                                return (
                                    <div onClick={(event) => comboHandler(event, ["clickPageNum",itemValue])} className={currentPage === itemValue ? "page-num current-page-num" : "page-num"}>
                                        <div className="pg-num-pointer-event" id={"pg-num"+String(itemValue)} style={currentPage === itemValue ? {color: "white"} : {color: "black"}}>{itemValue}</div>
                                    </div>
                                )
                            }
                            else if (itemValue + 2 === array.length && currentPage === array.length) {
                                return (
                                    <div onClick={(event) => comboHandler(event, ["clickPageNum",itemValue])} className={currentPage === itemValue ? "page-num current-page-num" : "page-num"}>
                                        <div className="pg-num-pointer-event" id={"pg-num"+String(itemValue)} style={currentPage === itemValue ? {color: "white"} : {color: "black"}}>{itemValue}</div>
                                    </div>
                            ...
                            else if (currentPage > itemValue + 1 || currentPage < itemValue - 1){
                                if (itemValue === 1) {
                                    return (
                                        <div className="page-num" id={"pg-num"+String(itemValue)} onClick={(event) => comboHandler(event, ["clickPageNum",itemValue])} style={currentPage === itemValue ? {color: "#be4217"} : {color: "black", position:"absolute", zIndex:"-1", opacity: "0"}}>{itemValue}</div>
                                    )
                                }
                            }
                            else {
                ...
                <BsArrowRight style={(currentPage === 1 && pageNumbers.length > 1) || currentPage !== pageNumbers.length && pageNumbers.length !== 0? {fontSize: "26px", cursor: "pointer"} : {fontSize: "26px", opacity: "0", pointerEvents: "none"}} onClick={nextPageHandler}/>
            </div>
        </div>
        {filterPopup ? <MiniFilter filterPopup={filterPopup} handleFilterPopup={handleFilterPopup} filterChange={comboHandler} initialPrice={initialPrice} priceRangeValue={priceRangeValue} priceChange={comboHandler} vegNonVeg={veg_nonVeg.current[0]}/> : <MiniFilter filterPopup={false} handleFilterPopup={handleFilterPopup} filterChange={comboHandler} initialPrice={initialPrice} priceRangeValue={priceRangeValue} priceChange={comboHandler} vegNonVeg={veg_nonVeg.current[0]}/>}
