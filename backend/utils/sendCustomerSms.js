...
const sendCustomerSms = catchAsyncErrors(async (paramList) => {
  if (paramList.length == 2) {
    //case for sending otp
    var phoneNumber = paramList[0];
    const otpCode = paramList[1];
    var smsText = "Your OTP from optibite is: " + otpCode;
  }

  if (paramList.length == 3) {
    //case for sending order update for logged in users
    ...
    var smsText =
      ...
      orderAddress +
      ". Your order is created on " +
      onlyDate +
      ". You will be contacted soon. Thank you, from Sabji Land.";
  }

  if (paramList.length == 4) {
    //case for sending order update for guests
    var phoneNumber = paramList[0];
    var orderDate = paramList[1];
    var orderAddress = paramList[2];
