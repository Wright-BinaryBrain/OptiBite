...
exports.getAllProduct = catchAsyncErrors(async (req, res, next) => {
  ...
  if (req.query.rowsPerPage) {
    // to get with pagination
    const resultPerPage = req.query.rowsPerPage;
    const productCount = await Product.countDocuments();

    if (sorting == "AscendingAlphabetically") {
      //for sorted data
      var apiFeatures = new APIFeatures(Product.find().sort("Name"), req.query)
        .search()
        .filter()
        .pagination(resultPerPage);
    } else if (sorting == "DescendingAlphabetically") {
