...
exports.getUserOrders = async(req,res)=>{
    ...
    if(!user){
        ...
        const recommendation = await axios.get(`http://localhost:8000/rec/${name}`);
        // console.log();
        ...
        // get user orders
        const userOrders = await Order.find({user:user._id}).populate('productId');
        //loop through products and get recommendations
        const recs = [];
        for(let i=0;i<userOrders.length;i++){
            const response = await axios.get(`http://localhost:8000/rec/${userOrders[i].productId[0].Name}`)
